...
class SettingsFragment : Fragment() {
    ...
    private fun loadSettings() {
        binding.apply {
            // Download settings
            val savedPath = prefs.getString(Constants.PREF_DOWNLOAD_PATH,
                android.os.Environment.getExternalStoragePublicDirectory(android.os.Environment.DIRECTORY_DOWNLOADS).absolutePath + "/SnapLoad")
            tvDownloadPath.text = savedPath
            ...
            sliderConcurrentDownloads.value = prefs.getInt(Constants.PREF_CONCURRENT_DOWNLOADS, 2).toFloat()
    ...
    private fun setupListeners() {
        binding.apply {
            // Download path
            btnChangeDownloadPath.setOnClickListener {
                // In a real app, launch SAF directory picker
                requireContext().showToast(getString(R.string.feature_coming_soon))
            }

            // Default quality
            spinnerQuality.onItemSelectedListener = object : android.widget.AdapterView.OnItemSelectedListener {
            ...
            sliderConcurrentDownloads.addOnChangeListener { _, value, _ ->
            ...
            // Update yt-dlp
            btnUpdateYtdlp.setOnClickListener {
                lifecycleScope.launch {
                    try {
                        // POST to /update endpoint
                        requireContext().showToast(getString(R.string.updating_ytdlp))
                    } catch (e: Exception) {
                        requireContext().showToast(e.localizedMessage ?: getString(R.string.error_generic))
                    }
                }
            }
